...
{	
	public function Create($notacredito) 
	{
		//Calcular la cantidad de articulos
		$cantArticulos = 0;
	    $total_a_Pagar = 0;
            $borrar=array();
	    foreach ($notacredito->getArticulonotacreditos() as $key => $articulonotacredito) 
	    {
	    	$cantArticulos ++;
	    	$articulonotacredito->setNotacredito($notacredito);
	        $total_a_Pagar += $articulonotacredito->getPrecio() * $articulonotacredito->getCantidad();
                
                if ($articulonotacredito->getCantidad()==0){
                    $borrar[]=$articulonotacredito;
                }
	    }
            foreach ($borrar as $elim){
                $notacredito->removeArticulonotacredito($elim);
            }
            

	    $notacredito->setCantidaddearticulos($cantArticulos);
	    $notacredito->setMontototalapagar($total_a_Pagar);
		...
		$tipo = $notacredito->getTipo();
		if ($tipo == "Anulación de la operación" || $tipo == "Anulación por error en el RUC")
		{
			//Anular la venta
			$this->DeleteVenta($notacredito->getVenta());
		}
		else
		{
		    
	        $antiguaDeuda = $this->RemoveVentaRelatedEntities($notacredito->getVenta());
	        $this->CreateVentaRelated($notacredito->getVenta(), $notacredito);		    
		}
		return true;
	}
	...
	public function CreateVentaRelated($venta, $notacredito) 
	{
        if ($venta->getTotalrecibido() != 0) {
            if ($notacredito->getMontototalapagar() - $venta->getTotalrecibido() > 1) {
                $deuda = $this->CreateDeuda($venta, $notacredito);
            }
            $this->CreatePagoInicial($venta, $notacredito);
        }
        $em = $this->getEntityManager();
        //Crear la deuda asociada a la venta
    }

	function CreateDeuda($venta, $notacredito)
	{
        $em = $this->getEntityManager();
        ...
        $diferencia = $notacredito->getMontototalapagar() - $venta->getTotalrecibido();
        ...
        $deuda->setTotalapagar($notacredito->getMontototalapagar());
        $em->persist($deuda);
        $em->flush();
    ...
    public function GetByFechaRangoParamCount($fecha1, $fecha2) 
    {
        ...
        $qb->select('COUNT(e)')
                ->from('BackendBundle:NotaCredito', 'e')
